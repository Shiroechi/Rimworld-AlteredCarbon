...
namespace AlteredCarbon
{
	...
	public static class GraphicDatabaseHeadRecords
	{
		...
		public static List<HeadGraphicRecord> maleHeads = new List<HeadGraphicRecord>();

		public static List<HeadGraphicRecord> femaleHeads = new List<HeadGraphicRecord>();
			...
			if (heads.Count <= 0 || skull == null || stump == null)
			{
				...
				foreach (string text in headsFolderPaths)
				{
					foreach (string item in GraphicDatabaseUtility.GraphicNamesInFolder(text))
					{
						...
						if (head.gender == Gender.Female) femaleHeads.Add(head);
						else if (head.gender == Gender.Male) maleHeads.Add(head);
						heads.Add(head);
					}
				}
